...
export const DeviceSimulator: React.FC<DeviceSimulatorProps> = ({ children }) => {
  ...
  useEffect(() => {
    // Check if device is a desktop (min-width: 768px)
  ...
  if (!isDesktop) {
    // On real mobile devices, just render the app normally
    return <>{children}</>;
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-8 overflow-hidden relative font-sans">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-20%] left-[-10%] w-[70vw] h-[70vw] rounded-full bg-brand-primary/10 blur-[120px] mix-blend-screen"></div>
      ...
      {/* Sidebar Label */}
      <div className="absolute bottom-8 right-8 text-slate-500 dark:text-slate-500 text-sm font-medium flex items-center gap-2">
        <span className="w-2 h-2 rounded-full bg-brand-primary animate-pulse"></span>
        Mobile View Simulator Active
      </div>
    </div>
  );
};
